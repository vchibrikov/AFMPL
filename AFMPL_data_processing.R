...
data_height <- data %>%
  ...
  summarize(
    ...
    n_measurements = n(),
    n_fibers = n_distinct(afm_fiber_number)) %>%
  ungroup()
...
# Save the workbook
saveWorkbook(wb, "calculations_output/height_statistics.xlsx", overwrite = TRUE)
...
data_contour_length <- data %>%
  group_by(fraction, ripening_stage) %>%
  summarize(mean_contour_length_nm = mean(contour_length_px),
            sd_contour_length_nm = sd(contour_length_px),
            n_fibers = n()) %>%
  ungroup()

data_contour_length$mean_contour_length_nm <- data_contour_length$mean_contour_length_nm * image_length_nm/image_quality_px
data_contour_length$sd_contour_length_nm <- data_contour_length$sd_contour_length_nm * image_length_nm/image_quality_px
...
# Save the workbook
saveWorkbook(wb, "calculations_output/length_statistics.xlsx", overwrite = TRUE)
...
shape_factor <- data_shape_factor %>%
  ...
  summarize(mean_shape_factor = mean(shape_factor),
            sd_mean_shape_factor = sd(shape_factor),
...
sheet_name <- "path_coordinates"
xlsx_files <- list.files(file_path, pattern = "\\.xlsx$", full.names = TRUE)

data_list <- lapply(xlsx_files, function(file) {
  read_excel(file, sheet = sheet_name)
})

data.path.coordinates <- bind_rows(data_list)
...
for (segment_number in 1:segment_number) {
  ...
  saveWorkbook(wb, paste0("calculations_output/persistence_length_msed_", segment_number, "_segment_statistics.xlsx"), overwrite = TRUE)
...
saveWorkbook(wb, paste0("calculations_output/persistence_length_bcf_", segment_number, "_segment_statistics.xlsx"), overwrite = TRUE)
}
